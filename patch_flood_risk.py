...
path = r"core\risk_engine.py"
backup = r"core\risk_engine.py.bak"

with open(path, "r", encoding="utf-8") as f:
    content = f.read()

new_method = '''    def calculate_flood_risk(self, record):
...
pattern = re.compile(
    r"    def calculate_flood_risk\(self, record\):.*?\n(?=    def |\Z)",
    re.DOTALL,
)
...
if len(matches) != 1:
    print(f"ABORTED -- expected exactly 1 match for calculate_flood_risk, found {len(matches)}. No changes made.")
else:
